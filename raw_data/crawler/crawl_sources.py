...
Lưu mỗi nguồn:
  ...
  pages/<NN>_<slug>.png        — screenshot (chỉ với HTML; tắt bằng --no-shots)

Ví dụ:
    python raw_data/crawler/crawl_sources.py --name schiphol --input refer_file/schiphol1.txt
    python raw_data/crawler/crawl_sources.py --name schiphol --input refer_file/schiphol1.txt --fresh
"""
...
from base_crawler import now_iso
            ...
            for r in csv.DictReader(f):
                ...
                if url:
                    rows.append({"url": clean_url(url),
                                 "anchor": (r.get("name") or r.get("website_name") or "").strip(),
...
def crawl_one(ctx, src: dict, idx: int, pages_dir: Path, args) -> dict:
    url = src["url"]
    slug = f"{idx:02d}_" + slugify(src["anchor"] or urlparse(url).path)
...
def main() -> None:
    ...
    sources = parse_input(args.input)
    if not sources:
        raise SystemExit(f"Không tách được URL nào từ {args.input}")
